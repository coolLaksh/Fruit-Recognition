...
for lof_neighbors in range(10, 11):
    """
    Outlier detection
    """
    ...
    inlier_X_train, inlier_Y_train = oversample.fit_resample(inlier_X_train, inlier_Y_train)


    for pca_components in range(500, 501, 100):
        """
        ...
        for lda_components in range(19, 20):
            ...
            for knn_n in range(30,31,5):
                neigh = KNeighborsClassifier(n_neighbors=knn_n)
                neigh.fit(lda_X_train, inlier_Y_train)
                train_knn_labels = neigh.predict(lda_X_train)
                train_knn_labels_encoded = np.zeros((train_knn_labels.size, classes.size), dtype=int)
                train_knn_labels_encoded[np.arange(train_knn_labels.size),train_knn_labels] = 1 
                
                val_knn_labels = neigh.predict(lda_X_val)
                val_knn_labels_encoded = np.zeros((val_knn_labels.size, classes.size), dtype=int)
                val_knn_labels_encoded[np.arange(val_knn_labels.size),val_knn_labels] = 1 

                lda_X_train = np.c_[lda_X_train, train_knn_labels_encoded]
                lda_X_val = np.c_[lda_X_val, val_knn_labels_encoded]

                clf = LogisticRegression(max_iter=1000, random_state=seed).fit(lda_X_train, inlier_Y_train)
                ...
                if val_score > max_performance:
                    ...
                    print("Best: ", max_performance, "lof: ", lof_neighbors, "pca_n: ", pca_components, "lda_n: ", lda_components, "knn:", knn_n)
